...
const prepareJobDescription = async (jobDescription) => {
...
const createFallbackResult = async (resume, jobDescription) => {
  ...
  return {
    ...
    evaluationDetails: [
      hasError
        ? `Warning: This resume had processing issues. Results may be incomplete.`
...
export const analyzeResumes = async (resumes, jobDescription) => {
  try {
    ...
    return Promise.all(
      resumes.map(resume => createFallbackResult(resume, jobDescription))
    );
  }
};
...
export const generateScoreAnalysis = (score) => {
...
// Mock function for testing - simplified for JavaScript
export const mockAnalyzeResumes = async (resumes, jobDescription) => {
